...
import { draftQueue, loadMerchantMap, openPriceDb } from "../src/index.js";
...
test("collect to review stays unpublished when AUTO_PUBLISH is off", async () => {
  ...
  assert.equal(draft.publish.auto_publish, false);
  ...
  assert.equal(draft.candidates[0].hist_price.hist, "unknown");
  assert.equal(draft.candidates[0].hist_price.reason, "no_history");
...
test("a missing publisher id keeps needs_affiliate true through the queue", async () => {
  ...
  assert.equal(draft.candidates[0].hist_price.hist, "unknown");
  assert.equal(draft.candidates[0].hist_price.enrichment, null);
